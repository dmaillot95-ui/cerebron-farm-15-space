...
def payload_for(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        elif lname in {'system','system_prompt'}: payload[name]='Rigorous aerospace engineering. CLAIM<=EVIDENCE. SIMULATION!=TEST.'
...
mission=pathlib.Path('MISSION.md').read_text(encoding='utf-8')[:7000]
models=['huggingface-projects/llama-3.2-3B-Instruct','huggingface-projects/gemma-2-9b-it']
model=models[sum(ord(c) for c in role)%2]
...
result={'role':role,'model':model,'inference_success':bool(ok),'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','output':text if ok else None,'error':None if ok else meta.get('error'),'meta':meta}
pathlib.Path('out').mkdir(exist_ok=True)
pathlib.Path(f'out/{role}.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
